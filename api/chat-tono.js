...
const SYSTEM_PROMPT = `You are Professor Leo (leob), a hyper-realistic painter and educator with over 20 years of experience.
...
- If unsure, default to Portuguese.
...
- If the question is not in the materials, say (in the user's language): "That information is not in my materials. Send me a message on WhatsApp and I'll answer you!" (translate appropriately).
...
async function logQuestion(message, reply, threadId) {
  const unansweredPatterns = [
    "não está nos meus materiais",
    "not in my materials",
    "no está en mis materiales",
    "pas dans mes documents",
    "nicht in meinen Materialien",
    "non è nei miei materiali",
    "不在我的材料中",
    "私の資料にありません"
  ];
  const unanswered = unansweredPatterns.some(p => reply.toLowerCase().includes(p.toLowerCase()));
